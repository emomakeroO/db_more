...
class Decoder(nn.Module):
    def __init__(self, input_dim=512):
        ...
        # 动态计算输入通道，适配 ResNet18 (512) 或 ResNet50 (2048)
...
class ResNetAE(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, pooling='avgpool',
        ...
        self.alpha = alpha 
        self.beta = beta   
        self.gamma = gamma # 对应文本中的 eta (scaling factor)
        
        # 预处理类别频率 (用于 DCB Loss 公式中的 P(y_i))
        ...
        # 动态获取 Encoder 输出维度 (ResNet18=512, ResNet50=2048)
    ...
    def compute_dcb_loss(self, device):
        ...
        # 文本公式: sum_i log(sigma(b(y_i))) * P(y_i)
        # 加上 1e-6 保持数值稳定性，防止 log(0)
        prob_bias = torch.sigmoid(avg_bias)
        dcb_loss = torch.sum(class_probs * torch.log(prob_bias + 1e-6))
        
        # 注意：因为 log(0~1) 是负数，最小化这个 Loss 会让 bias 变得更小（更负）。
        # 这对于头部类（P大）是符合逻辑的：强行压低头部类的 Logits。
    ...
    def forward(self, x, class_idx=None):
        ...
        # 计算 Regularization Loss (对应文本 Eq. 1 的第二部分 L_ent)
        dcb_reg_loss = self.compute_dcb_loss(device) * self.dcb_weight
        
        # 返回:
        # logits: 供外部 CrossEntropyLoss 使用 (完成 L_cls 计算)
        # recon_loss: L_recon (MoRE)
        # gate_loss: L_gate (MoRE)
        # dcb_reg_loss: L_ent (DCB)
