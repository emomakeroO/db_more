...
def val_cifar():
    ...
    with ExitStack() as stack:
        ...
        for images, targets in test_loader:
            images, targets = images.cuda(), targets.cuda()
            logits, scores = get_scores_fn(model, images,odc)
    ...
    clean_str = 'ACC: %.4f' % (test_acc)
    print(clean_str)
    fp.write(clean_str + '\n')

    clean_str = 'many_acc: %.4f' % (many_acc)
    print(clean_str)
    fp.write(clean_str + '\n')

    clean_str = 'median_acc: %.4f' % (median_acc)
    print(clean_str)
    fp.write(clean_str + '\n')

    clean_str = 'low_acc: %.4f' % (low_acc)
    print(clean_str)
    fp.write(clean_str + '\n')
    ...
    # print:
    ood_detectoin_str = 'auroc: %.4f' % (auroc)
    print(ood_detectoin_str)
    fp.write(ood_detectoin_str + '\n')

    ood_detectoin_str = 'aupr: %.4f' % (aupr)
    print(ood_detectoin_str)
    fp.write(ood_detectoin_str + '\n')

    ood_detectoin_str = 'fpr95: %.4f' % (fpr95)
    print(ood_detectoin_str)
    fp.write(ood_detectoin_str + '\n')
    ...
    for tpr_level in [0.98, 0.95, 0.90, 0.80]:
        ...
        print('id数量')
        print(len(in_labels_above_score_threshold))
...
def get_msp_scores(model, images,odc):
    
    logits0, logits1, logits2= model(images)
    all_pred =  F.softmax(logits0, dim=1)
    med_tail_pred = F.softmax(logits1, dim=1)
    tail_pred = F.softmax(logits2, dim=1)
    
    all_msp = torch.zeros(logits1.size(0))
    for i in range(all_msp.size(0)):
        all_msp[i]= torch.sum(all_pred[i, :odc]) 
        
    med_tail_msp = torch.zeros(logits1.size(0))
    for i in range(med_tail_msp.size(0)):
        med_tail_msp[i]= torch.sum(med_tail_pred[i, :odc]) 

    tail_msp = torch.zeros(logits2.size(0))
    for i in range(tail_msp.size(0)):
        tail_msp[i]= torch.sum(tail_pred[i, :odc]) 
    msp    = all_msp +med_tail_msp  +tail_msp 
    logits =all_pred +med_tail_pred +tail_pred  

    return logits[:,odc:], msp
...
if __name__ == '__main__':
    ...
    parser.add_argument('--odc',  type=int, default=30)
    ...
    if args.model == 'ResNet18':
        model = ResNet18(num_classes=num_classes+odc).cuda()
    elif args.model == 'ResNet34':
        model = ResNet34(num_classes=num_classes+odc).cuda()
    ...
    # log file:
    test_result_file_name = 'test_results_%s.xls' % (args.metric)
