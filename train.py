...
from models.our_resnet_re_13_g_fre import ResNet18AE, ResNet34AE
from models.our_resnet_imagenet_re_g_fre import ResNet50AE
...
def get_args_parser():
    ...
    parser.add_argument('--data_root_path', '--drp', default="/home/gcx/桌面/Archie/EAT/datasets",
                        help='data root path')
    ...
    parser.add_argument('--model', '--md', default='ResNet18', choices=['ResNet18', 'ResNet34', 'ResNet50'],
    ...
    parser.add_argument('--decay_epochs', '--de', default=[58, 60], nargs='+', type=int,
    ...
    parser.add_argument('--odc', type=int, default=3, help='ood_classes')
    ...
    parser.add_argument('--save_root_path', '--srp', default="/home/gcx/桌面/Archie/EAT", help='data root path')
    # ddp
...
def create_save_path():
    ...
    dataset_str = '%s-%s-OOD%d' % (
        args.dataset, args.imbalance_ratio, args.num_ood_samples) if 'imagenet' not in args.dataset else '%s%d-lt' % (
        args.dataset, args.id_class_number)
    save_dir = os.path.join(args.save_root_path, 'LT_OOD_results', 'EAT_re_13_g_fre', dataset_str, args.model, exp_str)
    create_dir(save_dir)
...
def train(gpu_id, ngpus_per_node, args):
    ...
    if args.dataset == 'cifar10':
        num_classes = 10 + odc
        train_set = IMBALANCECIFAR10(train=True, transform=TwoCropTransform(train_transform),
        ...
        num_classes = 100 + odc
        train_set = IMBALANCECIFAR100(train=True, transform=TwoCropTransform(train_transform),
                                      imbalance_ratio=args.imbalance_ratio, root=args.data_root_path)
        test_set = IMBALANCECIFAR100(train=False, transform=test_transform, imbalance_ratio=args.imbalance_ratio,
                                     root=args.data_root_path)
    elif args.dataset == 'imagenet':
        num_classes = args.id_class_number + odc
        args.data_root_path = "/home/gcx/桌面/data"  

        train_set = LT_Dataset(
            args.data_root_path,
            os.path.join(args.data_root_path, 'ImageNet_LT_train.txt'),
            transform=TwoCropTransform(train_transform),
            subset_class_idx=np.arange(0, args.id_class_number))

        test_set = LT_Dataset(
            args.data_root_path,
            os.path.join(args.data_root_path, 'ImageNet_LT_val.txt'),
    ...
    if args.model == 'ResNet18':
        model = ResNet18AE(num_classes=num_classes).to(device)
    elif args.model == 'ResNet34':
        model = ResNet34AE(num_classes=num_classes).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, ((in_data, labels)) in enumerate(train_loader):
            ...
            tail_labels = torch.tensor([], dtype=torch.long, device=device)
            ood_tail_labels = torch.tensor([], dtype=torch.long, device=device)
            extra_data = torch.tensor([], dtype=torch.float32, device=device)
            extra_in_data = torch.tensor([], dtype=torch.float32, device=device)
            dcb_loss = g_loss = 0
            if tail_num != 0:
                ...
                for i in range(times):
                    tail_data = torch.cat([tail_data, tail], dim=0)
                    tail_labels = torch.cat([tail_labels, tail_label], dim=0)
                extra_data = torch.cat([in_data[:N_in][head_idx0][:round(tail_labels.size(0) / 2)],
                                        in_data[N_in:2 * N_in][head_idx0][:round(tail_labels.size(0) / 2)]], dim=0)
            ...
            all_logits0, all_logits1, all_logits2, recon_loss = model(all_data)

            lt_loss0 = F.cross_entropy(all_logits0[:2 * N_in], in_labels) + \
                       args.Lambda0 * F.cross_entropy(all_logits0[2 * N_in:],
                                                      torch.cat([tail_labels, ood_tail_labels], dim=0))
            lt_loss1 = F.cross_entropy(all_logits1[:2 * N_in], in_labels) + \
                       args.Lambda0 * F.cross_entropy(all_logits1[2 * N_in:],
                                                      torch.cat([tail_labels, ood_tail_labels], dim=0))
            ...
            if rank == 0 and batch_idx % 100 == 0:
                train_str = 'epoch %d batch %d (train): loss %.4f (%.4f,  %.4f, %.4f, %.4f) | lr %s' % (
                    epoch, batch_idx, loss.item(), lt_loss0.item(), lt_loss1.item(),
                    lt_loss2.item(), recon_loss.item(), current_lr)
                print(train_str)
                fp.write(train_str + '\n')
                fp.flush()
        ...
        if rank == 0:
            ...
            val_str = 'epoch %d (test): ACC %.4f (%.4f, %.4f, %.4f) | F1 %.4f | time %s' % (
                epoch, overall_acc, many_acc, median_acc, low_acc, f1, time.time() - start_time)
            print(val_str)
